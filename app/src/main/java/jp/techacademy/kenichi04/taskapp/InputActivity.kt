...
class InputActivity : AppCompatActivity() {
    ...
    private val mOnDateClickListener = View.OnClickListener {
        val datePickerDialog = DatePickerDialog(this,
                DatePickerDialog.OnDateSetListener { _, year, month, dayOnMonth ->
                    mYear = year
                    mMonth = month
                    mDay = dayOnMonth
    ...
    private fun addTask() {
        ...
        // データの保存・更新: 引数のオブジェクトが存在していれば更新、なければ追加
        realm.copyToRealmOrUpdate(mTask!!)
        realm.commitTransaction()

        realm.close()

        // AlarmManagerを使ってアラームを設定
        val resultIntent = Intent(applicationContext, TaskAlarmReceiver::class.java)
        resultIntent.putExtra(EXTRA_TASK, mTask!!.id)
        // PendingIntentの作成; Intentの一種で、すぐに発行させるのではなく特定のタイミングで後から発行させるIntent
        ...
        Log.d("TaskApp", "${calendar.timeInMillis}")
    }
